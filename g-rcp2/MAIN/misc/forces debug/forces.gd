...
func debug_output() -> void:
	var frac_scale:float = (Scale / 1.0)
	...
	$compress.scale = Vector3(0.02, get_parent().directional_force.y * frac_scale, 0.02)
	$compress.translation.y = $compress.scale.y / 2.0
	$longi.scale = Vector3(0.02,0.02,get_parent().directional_force.z * frac_scale)
	$longi.translation.z = $longi.scale.z / 2.0
	$lateral.scale = Vector3(get_parent().directional_force.x * frac_scale, 0.02, 0.02)
