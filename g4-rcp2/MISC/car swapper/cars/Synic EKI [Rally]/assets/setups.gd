...
@export var suspension_gravel:ViVeWheelSuspension
...
@onready var car_parent:ViVeCar = get_parent()
...
func _on_setup_1_pressed() -> void:
	...
	for i:ViVeWheel in wheels:
		...
		i.Suspension.ReboundDampening = 12.0
		i.Camber = 0.0
		...
		if i.name.begins_with("f"):
			#i.S_Stiffness = 70.0
			#i.S_Damping = 4.0
			i.Suspension.SpringStiffness = 70.0
			i.Suspension.CompressionDampening = 4.0
		elif i.name.begins_with("r"):
			#i.S_Stiffness = 45.0
			#i.S_Damping = 3.0
			i.Suspension.SpringStiffness = 45.0
			i.Suspension.CompressionDampening = 3.0
